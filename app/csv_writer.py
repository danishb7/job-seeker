"""Write each search run to results/jobs_<timestamp>.csv."""
...
def write_csv(
    ...
    """Write a CSV for this run and return its absolute path."""
...
def list_results(results_dir: Path = RESULTS_DIR) -> list[dict[str, object]]:
    """Return metadata for every saved CSV, newest first."""
    ...
    for path in results_dir.glob("jobs_*.csv"):
        try:
            ...
            items.append(
                {
                    "filename": path.name,
                    "created_at": _dt.datetime.fromtimestamp(stat.st_mtime).isoformat(
                        timespec="seconds"
                    ),
                    "rows": rows,
